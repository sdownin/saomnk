...
structure_model <- list(
  dv_bipartite = list(
    ...
    coCovars = list( ##**STRATEGY -- MONADIC CONSTANT COVARIATE EFFECTS **
      list(effect='altX',   parameter= 0, fix=T,dv_name=DV_NAME,interaction1='self$component_1_coCovar', x= component_payoffs ),
      ...
      list(effect='inPopX', parameter= 0, fix=T,dv_name=DV_NAME,interaction1='self$strat_2_coCovar', x= actor_strats[[2]] )#, #interaction1 = NULL
...
env2$search_rsiena_multiwave_run(
  structure_model, 
  waves=1,
  iterations = 50 * env2$M * round(sqrt(env2$N)), 
  rand_seed = 12345
)
