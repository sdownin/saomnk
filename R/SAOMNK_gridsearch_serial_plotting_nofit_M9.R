...
grid.dir <- '__SAOMNK_gridsearch_input_start0_M9__173714448607676'
## INPUT FILE
...
if (!dir.exists(dir_proj_plotting)) {
  ...
  plotted <- dir(dir_proj_plotting, pattern = '\\.png')
...
for (i in 1:length(simfiles)) {
  ...
  plt3 <- self$search_rsiena_multiwave_plot('K_4panel',
      thin_factor = 3,
      return_plot=TRUE,     # plot_dir = dir_proj_plotting,    # plot_file = sprintf('%s_K4panel_thin1.png',sim_filebase) 
  )[[1]] ## get first item of list of plots
  ...
  ##---------------------------------------------------------------
  plot_npds <- 6
  step_ids <- seq(1, plot_npds*self$M*self$N, by=self$M*self$N )
  loess_span <- .4
  ...
  plt4 <- ggplot(actutilstrat, aes(x=chain_step_id, y=utility, color=strategy, linetype = actor_id)) + 
  ...
  plotname4 <- sprintf('%s_util_pointline_strat_pd.png',sim_filebase) 
  ggsave(filename=file.path(dir_proj_plotting, plotname4), plt4, 
         height = 7, width = 9, dpi = 600, units = 'in')
  ...
  #
  plt5 <- ggplot(actutilstrat %>% filter(period <= plot_npds) %>%
  ...
  #
  plotname5 <- sprintf('%s_util_raincloud_pd.png',sim_filebase) 
  ...
  for (i in 1:(self$M*self$N)) {
    sys_snapshots[[i]] <- plot_bipartite_system_from_mat(self$bi_env_arr[,,i], i)
  }
  sys_snapshots[[length(sys_snapshots)+1]] <- plot_bipartite_system_from_mat(self$bi_env_arr[,,step_end], step_end)
  # sys_snapshots[[length(sys_snapshots)+1]] <- sys_snapshots[[length(sys_snapshots)]] ## duplicate last frame for longer holder
  
  # library(gganimate)
  # 
  # # Combine plots into a single ggplot-compatible dataframe
  # animdf <- do.call(rbind, lapply(seq_along(sys_snapshots), function(i) {
  #   data <- sys_snapshots[[i]]$data
  #   data$frame <- i
  #   data
  # }))
  # 
  # # Create an animation with gganimate
  # animation <- ggplot(animdf) +
  ...
  plt6 <- marrangeGrob(sys_snapshots, nrow = 1, ncol = 1)
  
  # Save to a PDF
  plotname6pdf <- sprintf('%s_system_snapshots.pdf', sim_filebase)
  ggsave(filename = file.path(dir_proj_plotting, plotname6pdf), plt6, width = 13, height = 7)
