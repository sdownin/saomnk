...
write.csv(paramgrid[1, c('ID','time_start','time_duration')], file=file.path(dir_job, 'paramgrid_index.csv'), row.names = F)
...
# for (i in 1:10) ##**TEST**
# {
output <- foreach(i = 1:10, .packages = "base", .verbose=T) %dopar% {
# output <- foreach(i = 1:nrow(paramgrid), , .packages = "base", .verbose=T) %dopar% {
