...
structure_model <- list(
  dv_bipartite = list(
    ...
    effects = list( ##**STRUCTURAL EFFECTS -- dyadic/network endogeneity sources**
      list(effect='density', parameter= -50, fix=T, dv_name=DV_NAME), ##interaction1 = NULL
      list(effect='inPop',   parameter= .0,  fix=T, dv_name=DV_NAME), #interaction1 = NUL
      list(effect='outAct',  parameter= .0, fix=T, dv_name=DV_NAME)#, #interaction1 = NULL
    ...
    coCovars = list( ##**STRATEGY -- MONADIC CONSTANT COVARIATE EFFECTS **
      ...
      list(effect='egoX',   parameter= 0,fix=T,dv_name=DV_NAME,interaction1='self$strat_1_coCovar', x= actor_strats[[1]] )#, #interaction1 = NULL
...
m1$search_rsiena_multiwave_run(
  ...
  iterations = 1 * m1$M * m1$N, 
  rand_seed = 12345
)


##**TODO** 
##**ADD** add these methods to search_rsiena_run()
##        m1_s1 <- SaomNkRSienaBiEnv_search_rsiena_model$new(structure_model)
##**remove multiwave**##

# 1.1.1. process the chain
m1$search_rsiena_multiwave_process_results()
# 1.1.2.1. plot the actor utility timeseries
m1$search_rsiena_multiwave_plot('utility_strategy_summary')  ## thin_factor = 1
# 1.1.2.2. plot the structural degree summary (K4) panel 
m1$search_rsiena_multiwave_plot('K_4panel')  ## thin_factor = 1
 

## 1.2. Search 2. LONG Run
m2 <- SaomNkRSienaBiEnv$new(environ_params)
m2$search_rsiena_multiwave_run(
  structure_model, 
  waves=1,
  iterations = 30 * m2$M * m2$N, 
  rand_seed = 12345
)
m2$search_rsiena_multiwave_process_results()
m2$search_rsiena_multiwave_plot('utility_strategy_summary') ## thin_factor = 1
m2$search_rsiena_multiwave_plot('K_4panel') ## thin_factor = 1
