---
title: 'SaoMNK Tutorial: Exogenous Shock Comparison'
author: "S. Downing"
date: "`r Sys.Date()`"
output:
...
```{r}
# component_int_mat_opp <- 1-component_int_mat
```
...
```{r}
theta_shocks <- list(
  list(effect_level=c('egoX_1','egoX_2'), parameter=c(.3, 0),
       portion = 2, shock_on=1, label='Early Shock 1'
  ),
  list(effect_level=c('egoX_1','egoX_2'), parameter=c(0, .5),
       portion = 1, shock_on=1, label='Early Shock 2'
  ),
...
# theta_shocks <- list(
#   list(effect_level=c('altX','egoX_1','egoX_2'), parameter=c(1, 1, 0), 
#        portion = 1, shock_on=1, label='Early Shock 1'
#   ),
#   list(effect_level=c('altX','egoX_1','egoX_2'), parameter=c(0, 0, 1), 
#        portion = 1, shock_on=1, label='Early Shock 2'
#   ),
#   list(effect_level=c('altX','egoX_1','egoX_2'), parameter=c(1, 0, 0), 
#        portion = 2, shock_on=0
#   )
# )
...
snapshot_ids <- c(1, ceiling((1:30)*4), dim(env1$bi_env_arr)[3]  )
...
## 1.12 Exogenous Shock Comparison Analyses


```{r}
tests1 <- env1$compute_performance_shocks()
tests1
```
...
# 2. Switch Order of Exogenous Shock
...
theta_shocks_2 <- list(
  ...
  list(effect_level=c('egoX_1','egoX_2'), parameter=c(.3, 0),
       portion = 2, shock_on=1, label='Late Shock 1'
  ),
  list(effect_level=c('egoX_1','egoX_2'), parameter=c(0, .5),
       portion = 1, shock_on=1, label='Late Shock 2'
  )
)
# theta_shocks_2 <- list(
#   list(effect_level=c('altX','egoX_1','egoX_2'), parameter=c(1, 0, 0), 
#        portion = 2, shock_on=0
#   ),
#   list(effect_level=c('altX','egoX_1','egoX_2'), parameter=c(0, 1, 0), 
#        portion = 1, shock_on=1, label='Late Shock 1'
#   ),
#   list(effect_level=c('altX','egoX_1','egoX_2'), parameter=c(1, 0, 1), 
#        portion = 1, shock_on=1, label='Late Shock 2'
#   )
# )
...
## 2.12 Exogenous Shock Comparison Analyses



```{r}
tests2 <- env2$compute_performance_shocks()
tests2
```
...
```{r}
# modlist <- c(
#   lapply(env1$theta_shocks,function(x)x$rsiena_model),
#   lapply(env2$theta_shocks,function(x)x$rsiena_model)
# )
# modlist <- list(
#   env1_h1 = env1$theta_shocks[[1]]$rsiena_model,
#   env1_h2 = env1$theta_shocks[[2]]$rsiena_model,
#   env2_h1 = env2$theta_shocks[[1]]$rsiena_model,
#   env2_h2 = env2$theta_shocks[[2]]$rsiena_model
# )
# screenreg(modlist, digits=3, single.row = F)
```
