...
structure_model <- list(
  dv_bipartite = list(
    ...
    effects = list( ##**STRUCTURAL EFFECTS -- dyadic/network endogeneity sources**
      ...
      list(effect='outAct',  parameter= -.2, dv_name=DV_NAME, fix=T ),
      list(effect='inPop',   parameter=  .4, dv_name=DV_NAME, fix=T ) #interaction1 = NUL
    ),
    ## COVARIATE EFFECTS
    coCovars = list( 
...
```{r}
theta_shocks <- list(
  list(effect_level=c('egoX_1','egoX_2'), parameter=c(1, 0),
       portion = 1, shock_on=1, label='Early Shock 1'
  ),
  list(effect_level=c('egoX_1','egoX_2'), parameter=c(0, 1),
...
```{r}
# shocks <- env1$add_gof_to_rsiena_shocks()
# shocks
```
