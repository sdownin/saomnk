...
##****************************************************##
## INPUT FILE
grid.input.file <- 'SAOMNK_gridsearch_input_basic_M9.xlsx'
## ITERATIONS
ITERATIONS <- 10000
##****************************************************##
...
paramgrid$time_start <- NA ## This field will hold TIME_START IDs for each model run in parallel
...
write.table(paramgrid[1, c('ID','time_start','time_duration')], 
...
#
output_foreach <- foreach(i = 1:nrow(paramgrid), .combine = 'rbind', .packages = "base", .verbose=T) %dopar% {
  ...
  scenario_filebase <-  sprintf('%s_%s_%s',grid.input.filebase, grid.input.ts, scenario$time_start)
  ...
  write.table(paramgrid[i, c('ID','time_start','time_duration')],  
              file=file.path(dir_job, 'paramgrid_index.csv'), 
              sep=",",
              row.names = F, 
              col.names = F, 
  ...
  paramgrid[i, c('ID','time_start','time_duration')]
} ##**end main loop**
