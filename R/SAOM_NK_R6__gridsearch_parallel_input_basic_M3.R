...
cat(sprintf('\nParallel foreach cluster cores = %s \n', num_cores))
...
for (i in 1:length(strats$name)) {
  ...
  for (j in 1:length(strats$parameter)) {
    for (k in 1:length(strats$x[[stratname]])) {
      .id <- length(parameters[[stratname]]) + 1
      parameters[[stratname]][[ .id ]] <- list( 
        parameter    = as.numeric(strats$parameter[ j ] ) , 
        x            = strats$x[[stratname]][[ k ]],
...
paramgrid$time_start <- NA ## This field will hold TIME_START  for each model run in parallel
...
## save full gridsearch index
saveRDS(paramgrid, file=file.path(dir_job, 'paramgrid_index_ALL.rds'))

# ## START TRACKING INDEX TO UPDATED WHILE JOB RUNS
# write.table(paramgrid[1, c('ID','time_start','time_duration','uuid')], 
#             file=file.path(dir_job, 'paramgrid_index.csv'), 
#             sep=",",
#             row.names = F, 
#             col.names=TRUE, 
#             append = F)
...
write.csv(inputs, file = file.path(dir_job, gsub('xls(x)?','csv', grid.input.file)) )
...
##
#########################################################
# output_foreach <- foreach(i = 1:7, .combine = 'rbind',  .packages = "base", .verbose=T) %dopar% {
#
output_foreach <- foreach(i = 1:nrow(paramgrid), .combine = 'rbind', .packages = "base", .errorhandling = "pass", .verbose=T) %dopar% {
  ...
  paramgrid$time_start[i] <- Sys.time()
  ...
  scenario_filebase <-  sprintf('%s_%s_%s',grid.input.filebase, grid.input.ts, scenario$uuid)
  ...
  ##----------------------
  ## 2. Structure Model: effects, strategies, components payoffs (only constant covariates currently)
  ##----------------------
  ...
  for (j in 1:length(eff_strategy_params)) {
    ...
    for (k in 1:length(effvals)) {
      ...
      structure_model$dv_bipartite$coCovars[[ .id ]] <- list(
        ...
        parameter= effvals[[ k ]]$parameter, 
        interaction1 = effvals[[ k ]]$interaction1,
        x =  rep( effvals[[ k ]]$x,  times= scenario$M / length(effvals[[ k ]]$x) ),  ## repeats [1,2,3,...,s,  1,2,3,...,s,  ...] for times=M/#strats
        fix=TRUE, ## restricted from user changes currently
        dv_name='self$bipartite_rsienaDV' ## restricted from user changes currently
      )
    }
  ...
  env1$search_rsiena_multiwave_run(
    ...
    file_output = sprintf('%s_%s', i ,scenario_filebase) 
  )
  # Process results
  ...
  saveRDS(env1, file = file.path(dir_job_results, sprintf('%s_%s.rds', i , scenario_filebase) ) )
  cat("done.\n")
  ##
  
  ## UDPATE JOB INDEX
  paramgrid$time_duration[i] <-   Sys.time() - paramgrid$time_start[i]
  # write.table(paramgrid[i, c('ID','time_start','time_duration','uuid')],  
  #             file=file.path(dir_job, 'paramgrid_index.csv'), 
  #             sep=",",
  #             row.names = F, 
  #             col.names = F, 
  #             append = TRUE )
  # # write.csv(paramgrid[i,c('ID','time_start','time_duration')], file=file.path(dir_job, 'paramgrid_index.csv'), row.names = F, append = i>1 )
  # # write.csv(paramgrid[i,c('ID','time_start','time_duration')], file=file.path(dir_job, sprintf('paramgrid_index_%s.csv',i)), row.names = F )
  # ## end
  
  
  ## summarize results for this row returned from the whole foreach parallel loop 
  paramgrid[i, c('ID','time_start','time_duration','uuid')]
  
} ##**end main loop**


## Check parallel output
write.csv(output_foreach, file.path(dir_job, 'paramgrid_index__foreach_parallel_output.csv'), row.names = F)
