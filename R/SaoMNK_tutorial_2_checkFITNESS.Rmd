...
dyad_cov_X <- matrix(runif(n = environ_params$N*environ_params$M, min = 0, max= 1), nrow=environ_params$M)
...
structure_model <- list(
  dv_bipartite = list(
    ...
    coDyadCovars = list(
      ...
      list(effect='X',   parameter= 1, dv_name=DV_NAME, fix=T,
...
```{r,  fig.height=13, fig.width=10, fig.dpi=1000}
##
...
structure_model <- list(
  dv_bipartite = list(
    ...
    varDyadCovars = list(),
    interactions = list(
      list(effect='egoX|XWX',   parameter= 1, dv_name=DV_NAME, fix=T,
           interaction1='self$strat_1_coCovar',
           interaction2='self$component_1_coDyadCovar'
      ),
...
step_ids <- acpds[c(1:min(3, length(acpds)-1),length(acpds))]
#
step_id <- step_ids[ 2 ]
  ...
  # for (i in 1:min(3, self$M)) {
    ...
    z <- t(sapply( 1:nrow(dist_counterfac),  function(config_i) {
        ...
        c(dist=sum(x!=0),
          ...
          fitness=fit, 
          utility=util,
          degree= sum(as.numeric(start_actor_config))
          )
      })) %>% tidyr::as_tibble() %>% 
      mutate(start_str=start_actor_config_str, 
             ...
             add = as.numeric(add),
             degree_high = as.numeric(degree) > median(as.numeric(degree)),
             high_dist = dist > median(dist, na.rm=T)
             ) 
z$repertoire <- apply(z[,c('add','drop','dist','degree_high')], 1, function(x)paste(x, collapse = '|'))
z$add_by_dist <- apply(z[,c('add','dist')], 1, function(x)paste(x, collapse = '|'))
z$add_by_high_dist <- apply(z[,c('add','high_dist')], 1, function(x)paste(x, collapse = '|'))
    
    

plt <- z %>% ggplot(aes(x=utility, y=fitness, color=factor(repertoire))) +
  geom_smooth(alpha=.1, method='lm') +
  geom_point(alpha=.2) +
  facet_grid(factor(dist) ~ . , scales='free_y') +
  geom_vline(aes(xintercept = mean, color=factor(dist)), 
             data=z%>%group_by(dist)%>%summarize(mean=mean(utility))) +
# facet_grid(.~dist) +
  theme_bw() + 
  ggtitle(sprintf('Actor %s Step %s Fitness as function of Utility\nby configuration distance (number of tie changes from current config.)', actor_i, step_id))
...
print(summary(fitd))
#
(fitah <- lm(fitness ~ utility*factor(add), data=z %>%filter(dist > median(dist, na.rm=T)) ))
print(summary(fitah))
#
(fital <- lm(fitness ~ utility*factor(add), data=z %>%filter(dist <= median(dist, na.rm=T)) ))
print(summary(fital))
#
(fitdh <- lm(fitness ~ utility*factor(drop), data=z %>%filter(dist > median(dist, na.rm=T)) ))
print(summary(fitdh))
#
(fitdl <- lm(fitness ~ utility*factor(drop), data=z %>%filter(dist <= median(dist, na.rm=T)) ))
print(summary(fitdl))
write.csv(z, file=sprintf('test_utility_fitness2_step_id%s.csv',step_id), row.names=F)
```
...
z$add_by_dist <- apply(z[,c('add','dist')], 1, function(x)paste(x, collapse = '|'))
z$add_by_high_dist <- apply(z[,c('add','high_dist')], 1, function(x)paste(x, collapse = '|'))
z$add_drop_dist <- apply(z[,c('add','drop','dist')], 1, function(x)paste(x, collapse = '|'))
    

plt <- z %>% ggplot(aes(x=utility, y=fitness, color=factor(add_drop_dist))) +
  geom_smooth(alpha=.1, method='lm') +
  geom_point(alpha=.2) +
  facet_grid(factor(dist) ~ . , scales='free_y') +
