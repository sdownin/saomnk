...
# .uuid <- UUIDgenerate()
##  DIRECTORY
grid.dir <- sprintf('_SAOMNK_sensitivity_start0_rand%s__%s', nreps , .ts )
  ...
  structure_model <- list(
    dv_bipartite = list(
      ...
      coCovars = list( ##**STRATEGY -- MONADIC CONSTANT COVARIATE EFFECTS **
        list(effect='altX',   parameter= 20, fix=T,dv_name=DV_NAME,interaction1='self$component_1_coCovar', x= component_payoffs ),
        list(effect='egoX',   parameter= .02,fix=T,dv_name=DV_NAME,interaction1='self$strat_1_coCovar', x= actor_strats_list[[1]] ), #interaction1 = NULL
...
for (i in 1:length(sensi_files)) {
  ...
  lseed[[i]] <- run$seed 
  lutdf[[i]] <- run$utdf
  lutdfdiff[[i]] <- run$utdfdiff 
  lact[[i]] <- run$act
  lact_effs[[i]] <- run$act_effs 
  lproj_ts[[i]] <- run$proj_ts
  lrep_uuid[[i]] <- run$rep_uuid
  #
  dunn.tests[[i]] <- run$dunn.test
  dunn.tests.h1[[i]] <- run$dunn.test.h1
  dunn.tests.h2[[i]] <- run$dunn.test.h2
}
...
print( combined_plot )
ggsave(sprintf('%s_utility_summary_compare4seeds_color_strategy.png', .ts ), 
       combined_plot,  device = 'png',
       height = 8, width = 14, units = 'in', dpi=800)
...
utdf2 <- utdf %>% filter(period %in% pdskeep)
plt.summary <- utdf2 %>% 
  ...
  geom_vline(aes(xintercept = mean, color=strategy), 
               data=utdf2%>%group_by(strategy,period,seed)%>%summarize(mean=mean(utility)),
               linewidth=.8, linetype=3) + 
  geom_vline(aes(xintercept = mean, color=strategy), 
             data=utdf2%>%group_by(strategy,period)%>%summarize(mean=mean(utility)),
             linewidth=2) + 
  geom_vline(xintercept=0, linetype=2) +
  facet_grid(period ~ ., scales='free_y') +
  theme_bw() + ggtitle('Utility')
ggsave(sprintf('%s_utility_mean_facet_period_color_strategy.png', .ts ), 
       plt.summary,  
...
actdf2 <- actdf %>% filter(period %in% pdskeep)
plt.ent <- actdf2 %>%
    ...
    geom_vline(aes(xintercept = mean, color=strategy, linetype=strategy), 
               data=actdf2%>%group_by(strategy,period)%>%summarize(mean=mean(entropy)),
               linewidth=2) + 
    geom_vline(xintercept=0, linetype=2) +
    facet_grid(period ~ ., scales='free_y') +
    theme_bw() + ggtitle('Entropy')
ggsave(sprintf('%s_entropy_facet_period_color_strategy.png', .ts ), 
       plt.ent,  
       height = 10, width = 6, units = 'in', dpi=600)
print(plt.ent)
...
plt.gm <- utdf %>% group_by(strategy,seed) %>% summarize(utility=mean(utility,na.rm=T)) %>%
  mutate(rank=factor(order(utility, decreasing = TRUE))) %>%
  ggplot(aes(x=utility, color=strategy, fill=strategy)) +
  geom_density(alpha=.2) + 
  geom_vline(aes(xintercept = mean, color=strategy), data=utdf%>%group_by(strategy)%>%summarize(mean=mean(utility))) +
  # facet_grid( rank ~ . ) +
  theme_bw() + ggtitle('Global Means')  
print(plt.gm)
ggsave(sprintf('%s_global_means_by_strategy.png', .ts ), 
       plt.gm,  
...
ralllist <- list()
for (i in 1:length(seeds)) {
  z <- utdf %>% filter(seed == seeds[i] ) %>% group_by(strategy) %>% 
...
print(plt.rall)
ggsave(sprintf('%s_seed_rank_count_by_strategy.png', .ts ),
       plt.rall, 
       height = 6, width = 6, units = 'in', dpi=600)
...
print(plt.r)
ggsave(sprintf('%s_period_rank_count_by_strategy.png', .ts ),
       plt.r, 
       height = 6, width = 6, units = 'in', dpi=600)
...
for (i in 1:length(seeds)) {
  z <- utdf %>% 
    mutate(half=ifelse(period < median(period), 'H1','H2')) %>% 
    filter(seed == seeds[i] ) %>% 
    group_by(strategy, half) %>% 
    summarize(
      mean=mean(utility), 
  ...
  z1 <- z %>% filter(half=='H1')
  z2 <- z %>% filter(half=='H2')
  z1$rank <- factor(order(z1$mean, decreasing = T))
  z2$rank <- factor(order(z2$mean, decreasing = T))
  ralllist2[[i]] <- z1 %>% bind_rows(z2)
}
ralldf2 <- rbindlist(ralllist2)

plt.rall2 <- expand.grid(rank=levels(ralldf2$rank), strategy=unique(ralldf2$strategy), half=c('H1','H2')) %>%
  left_join(ralldf2, by=c('rank','strategy','half')) %>%
  group_by(strategy, half, rank) %>%
  ...
  ggtitle(sprintf('Count of Replication (seed) Utility Rank by Strategy \n(seeds = %s, periods = %s )', nreps, length(unique(utdf$period)))) 
print(plt.rall2)
ggsave(sprintf('%s_splithalf_seed_rank_count_by_strategy.png', .ts ),
       plt.rall2, 
       height = 6, width = 6, units = 'in', dpi=600)
...
zsigtab <- dtlong %>% group_by(comparisons) %>% 
  summarize(sig_abs_z2 = sum(abs(Z) > 2)/ n())
#
zcnts <- apply(zsigtab[,-1], 1, function(x)paste(c('Prop(|Z|>2) = '), round(x,3), sep=''))
...
#
dzplot <- dtlong %>%  ggplot(aes(x=abs(Z), color=comparisons, fill=comparisons, linetype=comparisons)) + 
  ...
  geom_vline(aes(xintercept = mean, color=comparisons, linetype = comparisons), 
             data=dtlong%>%group_by(comparisons)%>%summarize(mean=mean(abs(Z)))) +
  ...
  ggtitle(sprintf('Pairwise Dunn Test of Avg. Utility by Strategy from %s replications\n%s',  nreps,
                  paste(apply(zcnts, 2, function(x)paste(x[1],':', paste(x[-1], collapse = ';  '))), collapse = '\n' )))
print(dzplot)
ggsave(sprintf('%s_dunn_test_abs_Z_scores_by_strategy.png', .ts ),
       dzplot, 
