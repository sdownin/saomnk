...
```{r}
env3$preview_effects(structure_model, filter=FALSE)
```



```{r}
env3$preview_effects(structure_model, filter=TRUE)
```
...
```{r}
## Run Rsiena search using variable parameters in theta_matrix
env3$bi_env_arr[,, c(1,2,dim(env3$bi_env_arr)[3]) ]
```
...
env3$plot_actor_utility_strategy_summary()
```
...
for (i in 1:length(snapshot_ids)) {
  step <- snapshot_ids[ i ]
  mat <- env3$bi_env_arr[,,step]
  env3$plot_bipartite_system_from_mat(mat, step)
}
```
