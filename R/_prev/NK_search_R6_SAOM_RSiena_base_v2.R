...
SAOM_NK_RSiena <- R6Class(
  ...
  ##
    ...
    # RSIENA
    local_search_rsiena_init = function(iterations, show_effects_doc=FALSE, siena_phase2_nsub=1, siena_n2start_scale=1) {
      ...
      self$plot_bipartite_system_from_mat(new_bi_env_mat, iterations, plot_save = T)
    ...
    local_search_rsiena_run = function(iterations, n_snapshots=3, plot_save = TRUE, overwrite=TRUE, siena_phase2_nsub=1, siena_n2start_scale=1) {
  ...
  plot_bipartite_system_from_mat = function(bipartite_matrix, RSIENA_ITERATION, plot_save = FALSE) {
    ...
    component_labels <- generate_component_labels(N)
    
    # 1. Bipartite network plot using ggraph with vertex labels
    ig_bipartite <- igraph::graph_from_biadjacency_matrix(bipartite_matrix)
...
#
md1 <- SAOM_NK_RSiena$new(M = 20, N = 10, BI_PROB = .2, sim_name = '_TESTrsiena_')
md1$local_search_rsiena_run(iterations=4000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 1)
md1$plot_rsiena_sim_stability()
#
