...
SAOM_NK_Enhanced <- R6Class(
  ...
  public = list(
    ...
    fitness_history = NULL,  # fitness timeseries for plotting
    ...
    plot_fitness_progress = function() {
      ...
      ggplot(fitness_df, aes(x = Iteration, y = Fitness, color = factor(Actor))) +
        geom_line() +
...
saom_nk_enhanced$local_search_saom_batchrun(batches = 3, batch_iters = 4, plot_save = T)
...
## Simple environment 
env_simple <- SAOM_NK_Enhanced$new(M = 12, N = 8, BI_PROB = .1, sim_name = '_ENV_SIMPLE_')
env_simple$local_search_saom_batchrun(batches = 10, batch_iters = 5, plot_save=TRUE) 

## Mid environment 
env_mid <- SAOM_NK_Enhanced$new(M = 12, N = 12, BI_PROB = .1, sim_name = '_ENV_MID_')
env_mid$local_search_saom_batchrun(batches = 10, batch_iters = 5, plot_save=TRUE)  

## Complex environment 
env_complex <- SAOM_NK_Enhanced$new(M = 12, N = 18, BI_PROB = .1, sim_name = '_ENV_COMPLEX_')
env_complex$local_search_saom_batchrun(batches = 10, batch_iters = 5, plot_save=TRUE) 

## Very Complex environment 
env_verycomplex <- SAOM_NK_Enhanced$new(M = 12, N = 30, BI_PROB = .1, sim_name = '_ENV_VERYCOMPLEX_')
