  ...
  public = list(
    ...
    include_effect_from_eff_list = function(eff) {
      ...
      if (eff$effect == 'density') {
        self$siena_effects <- includeEffects(self$siena_effects,  density, ## get network statistic function from effect name (character)
                                             name = eff$dv_name, initialValue = eff$initialValue,
                                             # interaction1 = eff$interaction1,
                                             fix = eff$fix)
      }
      if (eff$effect == 'transTriads') {
        self$siena_effects <- includeEffects(self$siena_effects,  transTriads, ## get network statistic function from effect name (character)
                                             name = eff$dv_name, initialValue = eff$initialValue,
                                             # interaction1 = eff$interaction1,
                                             fix = eff$fix)
      }
      if (eff$effect == 'inPop') {
        self$siena_effects <- includeEffects(self$siena_effects, inPop, ## get network statistic function from effect name (character)
                                             name = eff$dv_name, initialValue = eff$initialValue,
                                             # interaction1 = eff$interaction1,
                                             fix = eff$fix)
      }
      if (eff$effect == 'outAct') {
        self$siena_effects <- includeEffects(self$siena_effects, outAct, ## get network statistic function from effect name (character)
                                             name = eff$dv_name, initialValue = eff$initialValue,
                                             # interaction1 = eff$interaction1,
                                             fix = eff$fix)
      }
      if (eff$effect == 'cycle4') {
        self$siena_effects <- includeEffects(self$siena_effects, cycle4, ## get network statistic function from effect name (character)
                                             name = eff$dv_name,
...
SaomNkRSienaBiEnv <- R6Class( 
  ...
  public = list(
    ...
    ##
    add_siena_effects = function(objective_list) {
      
      if (is.null(self$siena_effects))
        stop('initiate self$siena_effects before adding effects.')
      ...
      for (i in 1:length(objective_list)) {
        ...
        for (j in 1:length(dv$effects)) {
          ...
          #                                 name = 'self$bipartite_sienaDV', initialValue = 0, fix=T)  ## effect parameter 
        }
    ...
    local_search_rsiena_init = function(objective_list,  
      ...
      self$add_siena_effects(objective_list)
      
    },
    
    
    #
    local_search_rsiena_execute_sim = function(iterations, siena_phase2_nsub=1, siena_n2start_scale=1) {
      ##-----------------------------
      ## RSiena Algorithm
      self$siena_algorithm <- sienaAlgorithmCreate(projname=sprintf('%s_%s',self$SIM_NAME,self$TIMESTAMP),
                                                   simOnly = T,
                                                   nsub = siena_phase2_nsub,
                                                   n2start = siena_n2start_scale * 2.52 * (7+sum(self$siena_effects$include)),
                                                   n3 = iterations )
      ...
      new_bi_env_igraph <- self$get_bipartite_igraph_from_siena_sim()
    ...
    #
    get_bipartite_matrix_from_siena_sim = function(sim_iteration = NULL) {
    ...
    #
    get_bipartite_igraph_from_siena_sim = function(sim_iteration = NULL) {
      new_bi_env_mat <- self$get_bipartite_matrix_from_siena_sim(sim_iteration)
    ...
    #
    get_siena_model_snapshots = function(n=4, plot_save = TRUE, return_list=TRUE) {
      #
    ...
    local_search_rsiena_run = function(objective_list, iterations, n_snapshots=3, 
                                       plot_save = TRUE, overwrite=TRUE, 
                                       siena_phase2_nsub=1, siena_n2start_scale=1,
                                       get_eff_doc = FALSE
                                       ) {
      if( overwrite | is.null(self$siena_model) ) {
        self$local_search_rsiena_init(objective_list, get_eff_doc=get_eff_doc)
        self$local_search_rsiena_execute_sim(iterations, 
                                             siena_phase2_nsub=siena_phase2_nsub, 
                                             siena_n2start_scale=siena_n2start_scale)
      } else {
      ...
      # self$get_siena_model_snapshots(n=n_snapshots)
    },

    # ## Batch of Multiple Simulation Extensions to 
    # local_search_rsiena_batchrun = function(batches=10, iterations=100, plot_save = TRUE, overwrite=FALSE, siena_phase2_nsub=1) {
    #   
    #   for (i in 1:batches) {
    #   
    ...
    #     } else {
    #       self$local_search_rsiena_init(iterations, siena_phase2_nsub=siena_phase2_nsub)
    #     }
    # 
    #    
...
# self$siena_effects <- setEffect(self$siena_effects, density, 
#                                 name = 'self$bipartite_sienaDV', initialValue = 0, fix=T)  ## effect parameter vs. initial value ?
# 
# # # 
# # self$siena_effects <- includeEffects(self$siena_effects, 
# #                                      inPop, 
...
# #                                 name = 'self$bipartite_sienaDV', initialValue = .5)  ## effect parameter vs. initial value ?
# # # 
# # self$siena_effects <- includeEffects(self$siena_effects, 
# #                                      outAct, 
# #                                      name = 'self$bipartite_sienaDV')
...
# #                                 name = 'self$bipartite_sienaDV', initialValue = -.2, fix=T)  ## effect parameter vs. initial value ?
# # #-------------------------------
# ##-----------------------------
# ## SOCIAL SPACE
# self$siena_effects <- includeEffects(self$siena_effects, 
...
# #                                      name = 'self$social_sienaDV')
# self$siena_effects <- setEffect(self$siena_effects, density,
#                                 name = 'self$social_sienaDV', initialValue = 0, fix=T)  ## effect parameter vs. initial value ?
# # ## Social transitivity
# # self$siena_effects <- includeEffects(self$siena_effects,
# #                                      transTriads,
# #                                      name = 'self$social_sienaDV')
# # self$siena_effects <- setEffect(self$siena_effects, transTriads,
# #                                 name = 'self$social_sienaDV', initialValue = 0.4, fix = T)  ## effect parameter vs. initial value ?
# # 
# # ##
# ##-----------------------------
...
# #                                      name = 'self$search_sienaDV')
# self$siena_effects <- setEffect(self$siena_effects, density,
#                                 name = 'self$search_sienaDV', initialValue = 0, fix=T)  ## effect parameter vs. initial value ?
# ##-----------------------------
# },
# ##-----------------
...
params <- list(M = 10, N = 20, BI_PROB = .11, sim_name = '_TESTrsiena_')
## Strategies sets the objective function as a linear combination of network stats across DVs
objective_list <- list(
  dv_social = list(
    dv_name = 'self$social_sienaDV',
    effects = list(
      list(effect='density', initialValue=0, fix=FALSE, dv_name='self$social_sienaDV'), #interaction1 = NULL
      list(effect='transTriads', initialValue=0, fix=T, dv_name='self$social_sienaDV')#, #interaction1 = NULL
      # list(effect='density', initialValue=0, fix=T),
    )
  ),
  dv_search = list(
    dv_name = 'self$search_sienaDV',
    effects = list(
      list(effect='density', initialValue=0, fix=FALSE, dv_name='self$search_sienaDV'), #interaction1 = NULL
      list(effect='transTriads', initialValue=0, fix=T, dv_name='self$search_sienaDV')#, #interaction1 = NULL
      # list(effect='density', initialValue=0, fix=T),
    )
  ),
  dv_bipartite = list(
    name = 'self$bipartite_sienaDV',
    effects = list(
      list(effect='density', initialValue=0, fix=FALSE, dv_name='self$bipartite_sienaDV')#, #interaction1 = NULL
      # list(effect='inPop', initialValue=0, fix=T, dv_name='self$bipartite_sienaDV'), #interaction1 = NULL
      # list(effect='outAct', initialValue=0, fix=T, dv_name='self$bipartite_sienaDV'), #interaction1 = NULL
      # list(effect='cycle4', initialValue=0, fix=T, dv_name='self$bipartite_sienaDV')#, #interaction1 = NULL
      # list(effect='density', initialValue=0, fix=T),
    )
  )
)
...
# m1$get_effects_doc()
m1$local_search_rsiena_run(objective_list, iterations=2000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 4)
m1$plot_rsiena_sim_stability()

m1$local_search_rsiena_run(objective_list, iterations=2000, n_snapshots =1, siena_phase2_nsub=2, siena_n2start_scale = 1)
m1$plot_rsiena_sim_stability()


# ##
# m1$local_search_rsiena_run(iterations=4000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 1)
# m1$plot_rsiena_sim_stability()
# m1$local_search_rsiena_run(iterations=400, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 1)
# m1$plot_rsiena_sim_stability()
...
m1$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 1)
m1$plot_rsiena_sim_stability()
#
m2 <- SAOM_NK_RSiena$new(M = 10, N = 20, BI_PROB = .11, sim_name = '_TESTrsiena_')
m2$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=2, siena_n2start_scale = 1)
m2$plot_rsiena_sim_stability()

#
ms1 <- SAOM_NK_RSiena$new(M = 20, N = 10, BI_PROB = .11, sim_name = '_TESTrsiena_')
ms1$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 1)
ms1$plot_rsiena_sim_stability()
#
ms2 <- SAOM_NK_RSiena$new(M = 20, N = 10, BI_PROB = .11, sim_name = '_TESTrsiena_')
ms2$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=2, siena_n2start_scale = 1)
ms2$plot_rsiena_sim_stability()



md1 <- SAOM_NK_RSiena$new(M = 20, N = 10, BI_PROB = .2, sim_name = '_TESTrsiena_')
md1$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 1)
md1$plot_rsiena_sim_stability()
#
md2 <- SAOM_NK_RSiena$new(M = 20, N = 10, BI_PROB = .2, sim_name = '_TESTrsiena_')
md2$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=2, siena_n2start_scale = 1)
md2$plot_rsiena_sim_stability()

# #
# mc1 <- SAOM_NK_RSiena$new(M = 12, N = 30, BI_PROB = .05, sim_name = '_TESTrsiena_')
# mc1$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = 1)
# mc1$plot_rsiena_sim_stability()
# #
# mc2 <- SAOM_NK_RSiena$new(M = 12, N = 30, BI_PROB = .05, sim_name = '_TESTrsiena_')
# mc2$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=2, siena_n2start_scale = 1)
# mc2$plot_rsiena_sim_stability()

# #
# ms2 <- SAOM_NK_RSiena$new(M = 20, N = 10, BI_PROB = .15, sim_name = '_TESTrsiena_')
# ms2$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=1, siena_n2start_scale = .1)
# ms2$plot_rsiena_sim_stability()


#
# m3 <- SAOM_NK_RSiena$new(M = 10, N = 20, BI_PROB = .15, sim_name = '_TESTrsiena_')
# m3$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=3, siena_n2start_scale = 1)
# m3$plot_rsiena_sim_stability()
#
# m4 <- SAOM_NK_RSiena$new(M = 10, N = 20, BI_PROB = .15, sim_name = '_TESTrsiena_')
# m4$local_search_rsiena_run(iterations=2000, n_snapshots =1, siena_phase2_nsub=4, siena_n2start_scale = 1)
# m4$plot_rsiena_sim_stability()


saomnkrsiena <- SAOM_NK_RSiena$new(M = 10, N = 20, BI_PROB = .15, sim_name = '_TESTrsiena_')
##
# saomnkrsiena$local_search_rsiena_run(iterations=10, siena_phase2_nsub=1) 
# saomnkrsiena$local_search_rsiena_run(iterations=100, siena_phase2_nsub=2)
# saomnkrsiena$local_search_rsiena_run(iterations=5000, siena_phase2_nsub=3)
saomnkrsiena$local_search_rsiena_run(iterations=1000, n_snapshots =2, siena_phase2_nsub=1, siena_n2start_scale = 1)
saomnkrsiena$plot_rsiena_sim_stability()
# saomnkrsiena$local_search_rsiena_run(iterations=1000, siena_phase2_nsub=5)
...
# ########################################################################
# sims = saomnkrsiena$siena_model$sims
# outlist <- list()
# difflist <- list()
# jaccardlist <- list()
# for(i in 1:length(sims)) {
