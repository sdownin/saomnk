...
# Function to calculate fitness using the NK landscape and interaction matrix
calculate_fitness <- function(N, landscape, interaction_matrix, current_position, power_key) {
  fitness_vector <- numeric(N)
  for (i in 1:N) {
    # Create sub-bit string based on the interaction matrix for component i
    sub_bit_string <- current_position[which(interaction_matrix[i, ] == 1)]
    
    # Convert sub-bit string to a binary index for landscape lookup
    index <- sum(sub_bit_string * power_key[1:length(sub_bit_string)]) + 1  # 1-based index
    
    # Get the fitness value from the landscape
    if (index > nrow(landscape)) {
...
# Function to evaluate all possible combinations and identify local peaks
##
...
evaluate_all_combinations <- function(N, landscape, power_key, interaction_matrix) {
  num_combinations <- 2^N
  result_matrix <- matrix(0, nrow = num_combinations, ncol = (2 * N) + 2)
  
  for (comb_index in 0:(num_combinations - 1)) {
    current_combination <- as.integer(intToBits(comb_index)[1:N])
    fitness_vector <- calculate_fitness(N, landscape, interaction_matrix, current_combination, power_key)
    result_matrix[comb_index + 1, 1:N] <- current_combination  # Store combination
    result_matrix[comb_index + 1, (N + 1):(2 * N)] <- fitness_vector  # Store fitness contributions
    result_matrix[comb_index + 1, (2 * N) + 1] <- mean(fitness_vector)  # Store average fitness
    
    # Check if the current combination is a local peak
    is_local_peak <- TRUE
    for (j in 1:N) {
      neighbor_combination <- current_combination
      neighbor_combination[j] <- 1 - neighbor_combination[j]  # Flip bit to get a neighbor
      neighbor_index <- sum(neighbor_combination * power_key) + 1
      
      if (neighbor_index > num_combinations) next
      if (result_matrix[comb_index + 1, (2 * N) + 1] < result_matrix[neighbor_index, (2 * N) + 1]) {
        is_local_peak <- FALSE
        break
      }
    }
    result_matrix[comb_index + 1, (2 * N) + 2] <- as.integer(is_local_peak)
  }
...
# Generate the interaction matrix, power key, and NK landscape
interaction_matrix <- generate_interaction_matrix(N, K)
power_key <- generate_power_key(N)
landscape <- generate_nk_landscape(N)

# Run the evaluation for all combinations
result_matrix <- evaluate_all_combinations(N, landscape, power_key, interaction_matrix)

# Print the results
cat("Result matrix:\n")
print(result_matrix)

# Count and display the number of local peaks
num_peaks <- sum(result_matrix[, (2 * N) + 2])
cat("The number of local peaks is:", num_peaks, "\n")
